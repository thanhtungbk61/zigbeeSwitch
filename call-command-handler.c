...
EmberAfStatus emberAfManagerServerCommandParse(EmberAfClusterCommand *cmd)
{
  bool wasHandled = false;
  if (cmd->mfgSpecific) {
    if (cmd->mfgCode == 0x10A2 && cmd->commandId == ZCL_PUT_ON_OFF_SCHEDULE_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t* OnOffscheduleStr;  // Ver.: always
      // Command is not a fixed length
      if (cmd->bufLen < payloadOffset + emberAfStringLength(cmd->buffer + payloadOffset) + 1u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      OnOffscheduleStr = emberAfGetString(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfManagerPutOnOffScheduleCallback(OnOffscheduleStr);
    } else if (cmd->mfgCode == 0x10A2 && cmd->commandId == ZCL_PUT_DIMMING_SCHEDULE_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t* dimmingScheduleStr;  // Ver.: always
      // Command is not a fixed length
      if (cmd->bufLen < payloadOffset + emberAfStringLength(cmd->buffer + payloadOffset) + 1u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      dimmingScheduleStr = emberAfGetString(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfManagerPutDimmingScheduleCallback(dimmingScheduleStr);
    } else if (cmd->mfgCode == 0x10A2 && cmd->commandId == ZCL_PUT_GROUP_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t* groupStr;  // Ver.: always
      // Command is not a fixed length
      if (cmd->bufLen < payloadOffset + emberAfStringLength(cmd->buffer + payloadOffset) + 1u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
    ...
    } else if (cmd->mfgCode == 0x10A2 && cmd->commandId == ZCL_PUT_HISTORICAL_EVENT_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t* HistoryStr;  // Ver.: always
      // Command is not a fixed length
      if (cmd->bufLen < payloadOffset + emberAfStringLength(cmd->buffer + payloadOffset) + 1u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      HistoryStr = emberAfGetString(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfManagerPutHistoricalEventCallback(HistoryStr);
    } else if (cmd->mfgCode == 0x10A2 && cmd->commandId == ZCL_PUT_REPORT_REAL_TIME_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint8_t* reportTime;  // Ver.: always
      // Command is fixed length: 8
      if (cmd->bufLen < payloadOffset + 8u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      reportTime = cmd->buffer + payloadOffset;
      wasHandled = emberAfManagerPutReportRealTimeCallback(reportTime);
    } else if (cmd->mfgCode == 0x10A2 && cmd->commandId == ZCL_PUT_DIMMING_COMMAND_ID) {
      uint16_t payloadOffset = cmd->payloadStartIndex;
      uint16_t dimming;  // Ver.: always
      // Command is fixed length: 2
      if (cmd->bufLen < payloadOffset + 2u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
      dimming = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
      wasHandled = emberAfManagerPutDimmingCallback(dimming);
    }
  }
  return status(wasHandled, true, cmd->mfgSpecific);
}
